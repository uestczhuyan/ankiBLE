...
void timeAppIndGattMsg( gattMsgEvent_t *pMsg )
{
  ...
  switch ( i )
  {
    ...
    case HDL_BATT_LEVEL_START:
      ...
      HalLcdWriteStringValue( "N-Battery%", pMsg->msg.handleValueNoti.value[0], 10, HAL_LCD_LINE_2 );
      break;
...
static void timeAppDisplayAlert( uint8 *pValue, uint8 len )
{
  ...
  if ( pValue[0] <= ALERT_CAT_ID_MAX && len >= 2 )
  {
    ...
    HalLcdWriteStringValue( (char *) timeAppAlertCatStr[pValue[0]], pValue[1], 10, HAL_LCD_LINE_1 );
    pValue += 2;
    len -= 2;
