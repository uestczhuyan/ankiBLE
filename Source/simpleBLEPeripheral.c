...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  SimpleProfile_AddService( GATT_ALL_SERVICES );  // Simple GATT Profile ankiÐèÒªÐÞ¸ÄµÄ·þÎñ
#if defined FEATURE_OAD
  ...
  // Setup the SimpleProfile Characteristic Values
  {
    ...
    //Ç°Ãæ3¾äÖ»ÊÇ¶Ô¶ÔÏóÖµµÄÉèÖÃ£¬Õâ¾ä´úÂë ½øÐÐÁË£¬ServiceµÄcharacteristicÉèÖÃ£¬Process Client Characteristis Configuration Change
  ...
  // Register for all key events - This app will handle all key events¡£ ×¢²á°´Å¥ÊÂ¼þ£¬OSAL´¦Àí°´Å¥ÊÂ¼þ
  ...
  // ¸Ã»Øµ÷±» simpleProfile_WriteAttrCB ·½·¨´¥·¢£¬simpleProfileCBs °üº¬´Ë·½·¨£¬ÔÚSimpleProfile_AddService µÄÊ±ºò×¢²áµ½GATTÐ­ÒéÕ»·þÎñÖÐ
  //Öµ¸Ä±äÖ®ºó£¬Í¨Öª PROFILES--->SimpleProfileCBs.simpleProfile_WriteAttrCB  È»ºóÔÙÍ¨ÖªAPP--->simpleBLEPeripheral_SimpleProfileCBs
  ...
  // Setup a delayed profile startup  ·¢ÆðBLEÁ¬½Ó³õÊ¼»¯Ïà¹Ø·þÎñ¡£ÏÂÃæ·½·¨¶Ô SBP_START_DEVICE_EVT ÊÂ¼þ½øÐÐ´¦Àí
...
{
  //tasksArr[] ÊÇ11¸öTaskµÄ×îºóÒ»¸ö£¬ÓÉOSAL ÏµÍ³½øÐÐµ÷¶È¡£ÓÐÊ±¼äÀ´ÁË£¬ÓÃ¸Ã·½·¨´¦Àí¡£³õÊ¼»¯OSAL_SimpleBLEperipheal
  ...
  {
    // Start the Device   Õâ¸öÊÇÉè±¸ÓÐ×´Ì¬±ä»¯²Å»áµ÷ÓÃµÄ»Øµ÷º¯Êý
...
static void rssiRead( int8 newRSSI )
{
  //½øÐÐÏà¹Ø´¦Àí
    ...
    if(lastRSSI >= RSSI_CHANGE && newRSSI < RSSI_CHANGE){
      ...
      HalLcdWriteStringValue( "RSSIo£º", -newRSSI, 10,  HAL_LCD_LINE_8 );
    }
...
/**
* ¸Ä±äµÆ¹âµÄº¯Êý£¬phoneStatus ÊÖ»ú×´Ì¬£¬isChange ÊÇ·ñ²¥·ÅÇÐ»»Ð§¹û¡£
* phoneStatus = -2 ±íÊ¾¶ÁÈ¡µ½ÁËcharacteristic1 µÄÊý×éÖµ¡£
* phoneStatus = -1 ±íÊ¾¶ÁÈ¡µ½ÁËcharacteristic2 µÄÊý×éÖµ¡£
*/
static void dataChange(int8 phoneStatus,uint8 isChange){
      ...
      //Ö»¶ÁÈ¡µ½Êý×é1µÄÊý¾Ý£¬ÏÈ²»¸Ä±äµÆ¹âÖ»¼ÇÂ¼ÏÂÀ´£¬µÈÊý×é2µÄÊý¾Ý±»¶Áµ½ºó²Å½øÐÐµÆ¹â¸Ä±äµÄ²Ù×÷¡£
      if(phoneStatus == -2){
